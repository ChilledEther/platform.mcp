...
<task type="auto">
  <name>Task 1: Externalize Template Registry</name>
  <files>internal/templates/templates.yaml, internal/templates/loader.go</files>
  <action>
    1. Create `internal/templates/templates.yaml` listing available templates and their metadata.
    2. Update `internal/templates/loader.go` to load this registry at runtime (or via `embed`).
    3. Update `pkg/scaffold` generators to use the registry for file lookups instead of hardcoded paths.
  </action>
  <verify>
    Tests in `internal/templates` pass and confirm YAML loading.
  </verify>
  <done>
    Templates are managed via YAML config.
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement ProjectGenerator Wiring</name>
